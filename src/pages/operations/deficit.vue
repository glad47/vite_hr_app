<template>
  <view class="deficit-container">
    <!-- Header - Fixed -->
    <view class="page-header">
      <text class="page-title">العجوزات</text>
      <text class="page-subtitle">إدارة طلبات العجوزات المالية</text>
    </view>
    ...
    >
      ...
      <view class="mode-section" v-if="isSupervisor">
        <view class="mode-switch">
          ...
          >
            <text class="mode-text">عجوزاتي</text>
          </view>
          <view 
            class="mode-btn" 
      ...
      <view class="search-section">
		...
		<view class="search-row">
          <view class="search-item">
            <text class="search-label">مبلغ العجز</text>
            <input 
              class="search-input" 
              v-model="queryParams.amount" 
              placeholder="مبلغ العجز"
              type="number"
            />
          </view>
          <view class="search-item">
      ...
      <!-- List Content -->
      <!-- Loading -->
      <view v-if="loading && deficitList.length === 0" class="state-box">
        <view class="spinner"></view>
        <text class="state-text">جاري التحميل...</text>
      </view>
      
      <!-- Empty -->
      <view v-else-if="!loading && deficitList.length === 0" class="state-box">
      ...
      <view v-else class="list-content">
        <view class="list-item" v-for="item in deficitList" :key="item.id">
          <!-- Item Header -->
        ...
        <view class="no-more" v-else-if="!hasMore && deficitList.length > 0">
    ...
    <uni-popup ref="formDialog" type="center" :mask-click="false">
      <view class="dialog-box">
        <view class="dialog-header">
          <text class="dialog-title">{{ isEdit ? 'تعديل العجز' : 'إضافة جديدة' }}</text>
        ...
        <scroll-view class="dialog-body" scroll-y>
          ...
          <!-- Amount -->
          <view class="form-field">
            <text class="field-label required">مبلغ العجز</text>
            <uni-easyinput
              v-model="form.amount"
...
import { 
  ...
  updateDeficit, 
  delDeficit,
  listSuperviseesDeficits,
  countPendingDeficits,
  approveDeficit,
  rejectDeficit
} from '@/api/api/Deficit'
...
export default {
  ...
  methods: {
    ...
    async loadPendingCount() {
      try {
        const res = await countPendingDeficits(this.currentUserId)
    ...
    async loadList() {
      ...
      try {
        ...
        if (this.currentMode === 'personal' || !this.isSupervisor) {
          ...
          response = await listDeficit(this.queryParams)
        } else {
          ...
          response = await listSuperviseesDeficits(this.currentUserId, params)
        }
        
        this.deficitList = response.rows || []
        this.total = response.total || 0
        this.hasMore = this.deficitList.length < this.total
      } catch (error) {
    ...
    async loadMore() {
      ...
      try {
        ...
        this.deficitList = [...this.deficitList, ...newItems]
        this.total = response.total || 0
        this.hasMore = this.deficitList.length < this.total
      } catch (error) {
    ...
    onDueYearChange(e) {
      ...
      this.queryParams.dueYear = this.dueYearOptions[this.dueYearIndex].value
    },
    
    onDueMonthChange(e) {
      this.dueMonthIndex = e.detail.value
      this.queryParams.dueMonth = this.dueMonthOptions[this.dueMonthIndex].value
    },
    ...
    async handleEdit(item) {
      this.isEdit = true
      try {
        const res = await getDeficit(item.id)
        const data = res.data
        this.form = {
          id: data.id,
          empId: data.empId,
    ...
    onDueYearFormChange(e) {
      ...
      this.form.dueYear = this.dueYearOptions[this.dueYearFormIndex].value
    },
    
    onDueMonthFormChange(e) {
      this.dueMonthFormIndex = e.detail.value
      this.form.dueMonth = this.dueMonthOptions[this.dueMonthFormIndex].value
    },
    ...
    async submitForm() {
      ...
      if (!this.form.amount || parseFloat(this.form.amount) <= 0) {
        uni.showToast({ title: 'يرجى إدخال مبلغ العجز', icon: 'none' })
        return
      }
    ...
    handleDelete(item) {
      uni.showModal({
        title: 'تأكيد الحذف',
        content: 'هل أنت متأكد من حذف هذا العجز؟',
        confirmText: 'حذف',
        confirmColor: '#dc2626',
        success: async (res) => {
          if (res.confirm) {
            try {
              await delDeficit(item.id)
