<template>
  <view class="dedection-container">
    <!-- Header - Fixed -->
    <view class="page-header">
      <text class="page-title">الخصومات</text>
      <text class="page-subtitle">إدارة طلبات الخصومات</text>
    </view>
    ...
    >
      ...
      <view class="mode-section" v-if="isSupervisor">
        <view class="mode-switch">
          ...
          >
            <text class="mode-text">خصوماتي</text>
          </view>
          <view 
            class="mode-btn" 
      ...
      <view class="search-section">
        <view class="search-row">
          <view class="search-item">
            <text class="search-label">سنة الاستحقاق</text>
            <picker @change="onDueYearChange" :value="dueYearIndex" :range="dueYearOptions" range-key="label">
          ...
          <view class="search-item">
            <text class="search-label">شهر الاستحقاق</text>
            <picker @change="onDueMonthChange" :value="dueMonthIndex" :range="dueMonthOptions" range-key="label">
              <view class="search-select">
                <text>{{ getDueMonthLabel(queryParams.dueMonth) || 'الكل' }}</text>
                <uni-icons type="arrowdown" size="12" color="#64748b"></uni-icons>
              </view>
            </picker>
          </view>
        ...
        <view class="search-row">
          <view class="search-item">
            <text class="search-label">مبلغ الخصم</text>
            <input 
              class="search-input" 
          ...
          <view class="search-item">
            <text class="search-label">اسم المشرف</text>
            <input 
              class="search-input" 
      ...
      <!-- List Content -->
      <!-- Loading -->
      <view v-if="loading && dedectionList.length === 0" class="state-box">
        <view class="spinner"></view>
        <text class="state-text">جاري التحميل...</text>
      </view>
      
      <!-- Empty -->
      <view v-else-if="!loading && dedectionList.length === 0" class="state-box">
      ...
      <view v-else class="list-content">
        <view class="list-item" v-for="item in dedectionList" :key="item.id">
          <!-- Item Header -->
          <view class="item-header">
            <view class="item-amount-badge">
              {{ item.amount || 0 }} ر.س
            </view>
          ...
          <view class="item-body">
            ...
            <view class="detail-row">
              <text class="detail-label">سنة الاستحقاق</text>
              <text class="detail-value">{{ item.dueYear || '-' }}</text>
            </view>
            
            <view class="detail-row">
              <text class="detail-label">شهر الاستحقاق</text>
              <text class="detail-value">{{ getDueMonthText(item.dueMonth) }}</text>
            </view>
            
            <view class="detail-row" v-if="item.supervisorName">
              <text class="detail-label">اسم المشرف</text>
              <text class="detail-value">{{ item.supervisorName }}</text>
            </view>
        ...
        <view class="no-more" v-else-if="!hasMore && dedectionList.length > 0">
    ...
    <uni-popup ref="formDialog" type="center" :mask-click="false">
      <view class="dialog-box">
        <view class="dialog-header">
          <text class="dialog-title">{{ isEdit ? 'تعديل الخصم' : 'إضافة جديدة' }}</text>
        ...
        <scroll-view class="dialog-body" scroll-y>
          ...
          <view class="form-field">
            ...
            <picker @change="onDueYearFormChange" :value="dueYearFormIndex" :range="dueYearOptions" range-key="label">
          ...
          <view class="form-field">
            ...
            <picker @change="onDueMonthFormChange" :value="dueMonthFormIndex" :range="dueMonthOptions" range-key="label">
              <view class="field-select">
                <text :class="{ placeholder: !form.dueMonth }">{{ getDueMonthText(form.dueMonth) || 'اختر الشهر' }}</text>
                <uni-icons type="arrowdown" size="14" color="#94a3b8"></uni-icons>
              </view>
            </picker>
          </view>
          ...
          <!-- Amount -->
          <view class="form-field">
            <text class="field-label required">مبلغ الخصم</text>
            <uni-easyinput
              v-model="form.amount"
...
import { 
  ...
  updateDedection, 
  delDedection,
  listSuperviseesDedections,
  countPendingDedections,
  approveDedection,
  rejectDedection
} from '@/api/api/dedection'
...
export default {
  ...
  methods: {
    ...
    async loadPendingCount() {
      if (!this.isSupervisor) return
      try {
        const res = await countPendingDedections(this.currentUserId)
    ...
    onDueYearChange(e) {
      this.dueYearIndex = e.detail.value
      const selected = this.dueYearOptions[this.dueYearIndex]
      this.queryParams.dueYear = selected ? selected.value : null
    },
    
    onDueMonthChange(e) {
      this.dueMonthIndex = e.detail.value
      const selected = this.dueMonthOptions[this.dueMonthIndex]
      this.queryParams.dueMonth = selected ? selected.value : null
    },
    ...
    handleReset() {
      this.queryParams.dueYear = null
      this.queryParams.dueMonth = null
      this.queryParams.amount = null
      this.queryParams.supervisorName = null
      this.queryParams.status = null
      this.dueYearIndex = 0
      this.dueMonthIndex = 0
      this.statusIndex = 0
      this.handleSearch()
    },
    ...
    async loadList() {
      ...
      try {
        ...
        if (this.currentMode === 'personal' || !this.isSupervisor) {
          ...
          response = await listDedection(this.queryParams)
        } else {
          ...
          response = await listSuperviseesDedections(this.currentUserId, params)
        }
        
        this.dedectionList = response.rows || []
        this.total = response.total || 0
        this.hasMore = this.dedectionList.length < this.total
      } catch (error) {
    ...
    async loadMore() {
      ...
      try {
        ...
        this.dedectionList = [...this.dedectionList, ...newItems]
        this.hasMore = this.dedectionList.length < (response.total || 0)
      } catch (error) {
    ...
    async handleEdit(item) {
      this.isEdit = true
      try {
        const res = await getDedection(item.id)
        const data = res.data
        this.form = {
          id: data.id,
          empId: data.empId,
          dueYear: data.dueYear,
          dueMonth: String(data.dueMonth),
          amount: data.amount,
    ...
    updateFormIndices() {
      const dueYearIndex = this.dueYearOptions.findIndex(o => o.value === String(this.form.dueYear))
      this.dueYearFormIndex = dueYearIndex >= 0 ? dueYearIndex : 0
      
      const dueMonthIndex = this.dueMonthOptions.findIndex(o => o.value === String(this.form.dueMonth))
      this.dueMonthFormIndex = dueMonthIndex >= 0 ? dueMonthIndex : 0
    },
    
    onDueYearFormChange(e) {
      this.dueYearFormIndex = e.detail.value
      const selected = this.dueYearOptions[this.dueYearFormIndex]
      this.form.dueYear = selected ? selected.value : null
    },
    
    onDueMonthFormChange(e) {
      this.dueMonthFormIndex = e.detail.value
      const selected = this.dueMonthOptions[this.dueMonthFormIndex]
      this.form.dueMonth = selected ? selected.value : null
    },
    ...
    async submitForm() {
      ...
      if (!this.form.dueYear) {
        uni.showToast({ title: 'يرجى اختيار سنة الاستحقاق', icon: 'none' })
        return
      }
      if (!this.form.dueMonth) {
        uni.showToast({ title: 'يرجى اختيار شهر الاستحقاق', icon: 'none' })
        return
      }
      if (!this.form.amount || this.form.amount <= 0) {
        uni.showToast({ title: 'يرجى إدخال مبلغ الخصم', icon: 'none' })
        return
      }
      ...
      try {
        ...
        // Set supervisor name automatically from current user
        data.supervisorName = this.currentUserName || ''
        ...
        if (this.isEdit) {
          await updateDedection(data)
          uni.showToast({ title: 'تم التعديل بنجاح', icon: 'success' })
        } else {
          await addDedection(data)
    ...
    async handleApprove(item) {
      uni.showModal({
        ...
        success: async (res) => {
          if (res.confirm) {
            try {
              await approveDedection(item.id)
    ...
    resetForm() {
      ...
      this.dueYearFormIndex = 0
      this.dueMonthFormIndex = 0
      this.showEmpList = false
      this.empSearchText = ''
      this.empListCurrentPage = 1
      this.scrollTop = 0
    ...
    getDueMonthLabel(value) {
      if (value === null || value === undefined) return null
      const option = this.dueMonthOptions.find(o => o.value === String(value))
      return option ? option.label : null
    },
    
    getDueMonthText(value) {
      if (value === null || value === undefined) return null
      const monthMap = {
        '1': 'يناير',
        '2': 'فبراير',
      ...
      return monthMap[String(value)] || null
    },
