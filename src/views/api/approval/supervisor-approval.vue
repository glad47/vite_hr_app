<template>
  <div class="app-container">
    <div class="page-header">
      <h2>مركز الموافقات - المشرف</h2>
      <p>جميع الطلبات المعلقة بانتظار موافقة المشرف (المرحلة الأولى)</p>
    </div>

    <!-- Supervisor Selector (for admin to switch between supervisors) -->
    <el-row :gutter="16" class="mb8">
      <el-col :span="8">
        <el-select v-model="supervisorId" placeholder="اختر المشرف" filterable @change="loadData" style="width: 100%">
          <el-option v-for="sup in supervisors" :key="sup.id" :label="sup.name + ' (' + sup.id + ')'" :value="sup.id" />
        </el-select>
      </el-col>
    </el-row>

    <!-- Counts Summary -->
    <el-row :gutter="16" class="counts-row" v-if="supervisorId">
    ...
    <!-- Filter & Actions Bar -->
    <el-row :gutter="10" class="mb8 toolbar" v-if="supervisorId">
      <el-col :span="6">
    ...
    <!-- Table -->
    <el-table v-if="supervisorId" :data="filteredItems" v-loading="loading" @selection-change="handleSelectionChange" border stripe style="width: 100%">
      ...
      <el-table-column label="الحالة" width="140" align="center">
        <template slot-scope="scope">
          <el-tag type="info" size="small">قيد الانتظار</el-tag>
        </template>
      </el-table-column>
    ...
    <el-empty v-if="!supervisorId" description="جاري تحميل بيانات المشرف..." />
...
<script>
import {
  getSupervisorPendingItems, getSupervisorPendingCounts,
  supervisorApprove, supervisorReject, supervisorBatchApprove, supervisorBatchReject
} from '@/api/api/approval'
import { listEmp } from '@/api/api/emp'
...
export default {
  ...
  methods: {
    async loadSupervisors() {
      try {
        const res = await listEmp({ isSupervisor: '1', pageSize: 9999 })
        this.supervisors = res.rows || []

        // Auto-select: find supervisor matching the logged-in user
        // Try matching by sys_user.user_id = hr_emp.id (if they match)
        const currentUserId = this.$store.getters.id
        const match = this.supervisors.find(s => s.id === currentUserId)
        if (match) {
          this.supervisorId = match.id
          this.loadData()
        ...
        // Otherwise admin must select manually from dropdown
      } catch (e) {
    ...
    async loadData() {
      ...
      try {
        ...
        this.$message.error('فشل تحميل البيانات')
      } finally {
        this.loading = false
      }
    },
    ...
    typeTagColor(type) {
      return ({ requests: '', additions: 'success', dedections: 'danger', dedectionWs: 'danger', advances: 'warning', punches: 'info', leaves: '' })[type] || ''
    },

    requestTypeLabel(type) {
      return ({ '0': 'خروج وعودة', '1': 'خروج مبكر', '2': 'تأخير في الحضور' })[type] || type
    },

    leaveTypeLabel(type) {
      return ({ '0': 'سنوية', '1': 'مرضية', '2': 'رسمية', '3': 'خاصة', '4': 'اضطرارية' })[type] || type
    },
    ...
    async approveItem(item) {
      try {
        ...
        if (e !== 'cancel') this.$message.error('فشلت العملية')
      }
    },

    rejectItem(item) {
      this.rejectTarget = item
      this.rejectBatch = false
      this.rejectNote = ''
    ...
    async confirmReject() {
      ...
      try {
        if (this.rejectBatch) {
          await supervisorBatchReject({
            items: this.selectedItems.map(i => ({ type: typeMap[i._type], id: i.id })),
            approverId: this.supervisorId,
            note: this.rejectNote
          })
        } else {
          await supervisorReject({
            type: typeMap[this.rejectTarget._type],
            id: this.rejectTarget.id,
            supervisorId: this.supervisorId,
            note: this.rejectNote
          })
        }
        this.$message.success('تم الرفض بنجاح')
        this.rejectDialogVisible = false
        this.loadData()
      } catch (e) {
        this.$message.error('فشلت العملية')
      }
    },
